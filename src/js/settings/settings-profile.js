...
export function _renderAccountSettings() {
  if (_guestMode) {
    ...
    const returnBtn = new View('button');
  ...
  const avatarHtml = _authUserInfo?.picture
  ...
  const avatar = RawHTML('<div class="relative group cursor-pointer" style="flex-shrink:0">' + avatarHtml +
  ...
  const signOutBtn = new View('button');
  ...
  const deleteBtn = new View('button');
  ...
  const guestBtn = new View('button');
