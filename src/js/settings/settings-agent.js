...
export function _renderAISettings() {
  ...
  const masterToggle = _settingToggle('AI Features', 'Master switch. When off, all AI models and features are disabled.',
    aiOn, function(on) {
  ...
  const masterGroup = _settingCard('Master', [masterToggle]);
  ...
  const providerBtns = [
    ...
    b.onTap(function() {
      ...
      b.el.parentElement.querySelectorAll('button').forEach(function(btn) {
        const isActive = btn.dataset.provider === name;
  ...
  const apiKeyInput = new window.View('input');
  ...
  apiKeyInput.el.addEventListener('change', function() {
    const key = apiKeyInput.el.value.trim();
  ...
  const systemPrompts = window.RawHTML(
    ...
    '<pre class="text-dim text-[0.72rem] whitespace-pre-wrap leading-relaxed font-mono">You are a helpful assistant with tools to search the web, find papers, fetch page content, bookmark posts, and navigate the app. Use tools when they would help answer the user\'s question.\\n\\nToday is <span class="text-accent">Wednesday, 2026-02-09</span>. The current time is <span class="text-accent">3:45 PM</span>.</pre></div>' +
