...
function _renderFeedSettings() {
  return `
    ...
    <div class="mb-8 pt-5 border-t border-border-subtle">
      <h3 class="text-white_ text-sm font-semibold mb-2">Quality Filter & Algorithm</h3>
      <p class="text-dim text-[0.8rem] mb-3">Manage the AI quality filter, scoring prompts, and personalized feed algorithm.</p>
      <div class="flex gap-2">
        <button onclick="window.location.hash='quality'" class="px-3 py-1.5 rounded-md text-[0.78rem] border border-border-input text-muted bg-card hover:border-accent hover:text-primary cursor-pointer transition-colors">Open Quality Filter</button>
        <button onclick="window.location.hash='algorithm'" class="px-3 py-1.5 rounded-md text-[0.78rem] border border-border-input text-muted bg-card hover:border-accent hover:text-primary cursor-pointer transition-colors">Open Algorithm</button>
      </div>
    </div>
  `;
}
...
function renderSettingsView() {
  ...
  if (pane) {
    const titles = { profile: 'Profile', appearance: 'Appearance', feed: 'Feed & Reading', tools: 'Tools', browser: 'Browser' };
