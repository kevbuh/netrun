...
/**
 * Create a container element for paper tab content.
 * Returns a DOM element with proper styling for absolute positioning and overflow.
 * @param {number} tabId - The tab ID
 * @returns {HTMLElement} The styled container div
 */
function _createPaperContainer(tabId) {
  const el = document.createElement('div');
  el.id = 'browse-paper-' + tabId;
...
export function _browseRestoreTabs() {
  try {
    ...
    if (raw) {
      ...
      if (!container) {
        logger.error('[browse] browse-content container not found');
        return false;
      }
      ...
      for (const savedWin of windows) {
        ...
        for (const saved of savedWin.tabs) {
          if (saved.blank) {
            const tab = { id: saved.id, url: '', title: 'New Tab', favicon: '', el: null, blank: true, lastVisited: saved.lastVisited || 0, backStack: saved.backStack || [], forwardStack: saved.forwardStack || [], origin: saved.origin || null };
          ...
          if (saved._historyPage) {
            const tab = { id: saved.id, url: 'netrun://history', title: 'History', favicon: '', el: null, blank: false, _historyPage: true, lastVisited: saved.lastVisited || 0, backStack: saved.backStack || [], forwardStack: saved.forwardStack || [], origin: saved.origin || null };
            if (saved.pinned) tab.pinned = true;
            if (saved.groupId != null) tab.groupId = saved.groupId;
            win.tabs.push(tab);
            continue;
          ...
          if (saved._chatPage) {
            const tab = { id: saved.id, url: saved.url || ('chat://' + (saved._chatThreadId || '')), title: saved.title || 'Chat', favicon: '', el: null, blank: false, _chatPage: true, _chatThreadId: saved._chatThreadId || null, lastVisited: saved.lastVisited || 0, backStack: saved.backStack || [], forwardStack: saved.forwardStack || [], origin: saved.origin || null };
            if (saved.pinned) tab.pinned = true;
            if (saved.groupId != null) tab.groupId = saved.groupId;
            win.tabs.push(tab);
            continue;
          ...
          if (saved.paper && saved.contentType) {
            const el = _createPaperContainer(saved.id);
            ...
            const tab = { id: saved.id, url: saved.url, title: saved.title || _browseTitleFromUrl(saved.url), favicon: _browseFaviconUrl(saved.url), el, blank: false,
                          paper: saved.paper, contentType: saved.contentType, arxivId: saved.arxivId || null, lastVisited: saved.lastVisited || 0, backStack: saved.backStack || [], forwardStack: saved.forwardStack || [], origin: saved.origin || null };
          ...
          const tab = { id: saved.id, url: saved.url, title: saved.title || _browseTitleFromUrl(saved.url), favicon: _browseFaviconUrl(saved.url), el, blank: false, deferred: shouldDefer, lastVisited: saved.lastVisited || 0, backStack: saved.backStack || [], forwardStack: saved.forwardStack || [], origin: saved.origin || null };
          if (saved.pinned) tab.pinned = true;
          if (saved.groupId != null) tab.groupId = saved.groupId;
          win.tabs.push(tab);
