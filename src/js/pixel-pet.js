...
export const CPX = 32; // canvas element pixels
export const S = CPX / G; // scale factor (2)
...
// ── Particles ──
export function drawParticle(ctx, type, x, y, frame) {
  const s = S;
  if (type === 'heart') {
...
// ── State machine ──
export let petState = 'idle', prevBaseState = 'idle';
export let petX = 200, petY = 400;
export let petTargetX = 300, petTargetY = 400;
export let petDir = 1;
export let petFrame = 0;
export let petEyeDir = 'center', petEyeTimer = 0;
export let petStateTimer = 0, petTempTimer = 0;
export let _petLoop = null;
export let _lastActivity = Date.now();
export let _lastScrollY = 0, _scrollSpeed = 0;
export let _mouseX = -1, _mouseY = -1;
export const _fleeTimer = 0;

// ── Drag state ──
export let _dragging = false;
export let _dragOffX = 0, _dragOffY = 0;
export let _dragPrevState = 'idle';
...
export function drawPetBed(ctx) {
  const s = S;
...
// ── Shadow drawing ──
export function drawShadow(ctx, yOff, floating) {
  const cx = CPX / 2;
...
export function isSidebarMode() { return getPetMode() === 'sidebar'; }
...
export let _sbDragFloating = false;
export let _sbDragReordering = false;
export let _sbDragGhost = null;
export let _sbDragStartY = 0;

export function onSidebarDragStart(e) {
  if (e.button !== 0) return;
  e.preventDefault();
  e.stopPropagation();
  _dragPrevState = petState;
...
export function onSidebarDragEnd(e) {
...
// ── Click handling ──
export let _lastClickTime = 0;
export let _dragStartX = 0, _dragStartY = 0;
export function onPetMouseDown(e) {
  if (e.button !== 0) return;
